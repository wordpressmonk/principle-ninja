...
class Customdomain extends MY_Controller {
	...
	public function __construct()
	{
		...
		/** Hook point */
		$this->hooks->call_hook('customdomain_construct');
	}
	...
	{
		/** Hook point */
		$this->hooks->call_hook('customdomain_index_pre');
		...
		$req_host = $server_scheme . "://" . $_SERVER['HTTP_HOST'];
		$request = get_domaininfo($req_host);

		$request_by = ($request['subdomain'] != '')? $request['subdomain'].'.'.$request['domain'] : $request['domain'];

		$site_content = $this->MSites->get_by_field_value('custom_domain', $request_by);
		if (count($site_content) > 0)
		{
			...
			foreach($raw->find('a') as $element)
			{
				...
				{
					$element->href = $server_scheme . '://' . $_SERVER['HTTP_HOST'] . '/' . $element->href;
				}
			}

			/** Forms using Sent api will need to have the app domain prepended to the action attr **/
			foreach ($raw->find('form[data-action="sentapi"]') as $element)
			{

				$element->action = $this->config->item('base_url') . ltrim($element->action, '/');
			}
			...
			/** Hook point */
			$this->hooks->call_hook('customdomain_index_post');
		...
		{
			/** Hook point */
			$this->hooks->call_hook('customdomain_index_error');
	...
	{
		/** Hook point */
		$this->hooks->call_hook('customdomain_destruct');
	}
